...
const Header = () => {
    ...
    useEffect(() => {
        ...
        if(token){
            ...
            if(decodedToken.exp * 1000 < new Date().getTime()) logout(router, dispatch);
        }
    ...
    <header className='w-full h-fit min-h-[73px] flex justify-center items-center flex-col bg-white border-b border-b-[#e6ebf4] md:min-h-[100px] md:px-5'>
        <div className='w-full h-fit px-4 py-4 flex justify-between items-center md:px-2'>
          <div className='flex justify-center items-center'>
            ...
            <Link href="/" onClick={() => {setLeftPosition(0); setTopPosition(0);setButtonWidth(homeBtnRef.current.getBoundingClientRect().width); }}> 
              <Image  src={AllKnowing} alt="" className='w-[40px] aspect-square'/>
            </Link>
          </div>
          ...
          <div className='relative w-fit flex justify-center items-center rounded-lg md:hidden'>
            ...
            <Link href="/"              ref={homeBtnRef}        onClick={(e) => {setLeftPosition(e.target.offsetLeft); }} className={`${(pathname === "/")              && ("text-white")} px-5 py-3 relative z-10 h-fit mr-5 rounded-lg font-medium text-center transition-all ease-in-out`}>Home</Link>
            <Link href="/chatBot"       ref={chatBtnRef}        onClick={(e) => {setLeftPosition(e.target.offsetLeft); }} className={`${(pathname === "/chatBot")       && ("text-white")} px-5 py-3 relative z-10 h-fit mr-5 rounded-lg font-medium text-center transition-all ease-in-out`}>Chat</Link>
            <Link href="/createImage"   ref={createImageBtnRef} onClick={(e) => {setLeftPosition(e.target.offsetLeft); }} className={`${(pathname === "/createImage")   && ("text-white")} px-5 py-3 relative z-10 h-fit mr-5 rounded-lg font-medium text-center transition-all ease-in-out`}>Create Image</Link>
            <Link href="/summarizeURL"  ref={summarizeBtnRef}   onClick={(e) => {setLeftPosition(e.target.offsetLeft); }} className={`${(pathname === "/summarizeURL")  && ("text-white")} px-5 py-3 relative z-10 h-fit mr-5 rounded-lg font-medium text-center transition-all ease-in-out`}>Summarize</Link>
            <Link href="/translator"    ref={translateBtnRef}   onClick={(e) => {setLeftPosition(e.target.offsetLeft); }} className={`${(pathname === "/translator")    && ("text-white")} px-5 py-3 relative z-10 h-fit mr-5 rounded-lg font-medium text-center transition-all ease-in-out`}>Translate</Link>
            <Link href="/context"       ref={contextBtnRef}     onClick={(e) => {setLeftPosition(e.target.offsetLeft); }} className={`${(pathname === "/context")       && ("text-white")} px-5 py-3 relative z-10 h-fit mr-5 rounded-lg font-medium text-center transition-all ease-in-out`}>Ask From Context</Link>
            {
              // (user) ? (
              //   <div className="flex">
              //       <button onClick={ handleLogout } className="font-medium bg-[rgb(217,217,217)] text-black px-4 py-2 rounded-md mr-2"></button>
              //       <Image src={user?.result?.picture} alt={user?.result?.name} className=" rounded-full w-10"/> 
              //   </div>
              // ) : (
              ...
              <div className='relative flex justify-center items-center w-full h-max'>
                <div style={{top: topPosition, width: "100%"}} className={`absolute bg-black h-[14%] transition-all ease-in-out duration-500`}></div>
                  <div className='w-full flex flex-col justify-center items-center '>
                    <Link href="/"               onClick={(e) => {setTopPosition(e.target.offsetTop); }} className={`${(pathname === "/")              && ("text-white")} ${showNav ? "left-0" : "left-[100px]"} w-full py-3 relative z-10 h-fit rounded-lg font-medium text-center transition-all ease-in-out duration-[100ms]`}>Home</Link>
                    <Link href="/chatBot"        onClick={(e) => {setTopPosition(e.target.offsetTop); }} className={`${(pathname === "/chatBot")       && ("text-white")} ${showNav ? "left-0" : "left-[200px]"} w-full py-3 relative z-10 h-fit rounded-lg font-medium text-center transition-all ease-in-out duration-[200ms]`}>Chat</Link>
                    <Link href="/createImage"    onClick={(e) => {setTopPosition(e.target.offsetTop); }} className={`${(pathname === "/createImage")   && ("text-white")} ${showNav ? "left-0" : "left-[300px]"} w-full py-3 relative z-10 h-fit rounded-lg font-medium text-center transition-all ease-in-out duration-[300ms]`}>Create Image</Link>
                    <Link href="/summarizeURL"   onClick={(e) => {setTopPosition(e.target.offsetTop); }} className={`${(pathname === "/summarizeURL")  && ("text-white")} ${showNav ? "left-0" : "left-[400px]"} w-full py-3 relative z-10 h-fit rounded-lg font-medium text-center transition-all ease-in-out duration-[400ms]`}>Summarize</Link>
                    <Link href="/translator"     onClick={(e) => {setTopPosition(e.target.offsetTop); }} className={`${(pathname === "/translator")    && ("text-white")} ${showNav ? "left-0" : "left-[500px]"} w-full py-3 relative z-10 h-fit rounded-lg font-medium text-center transition-all ease-in-out duration-[500ms]`}>Translate</Link>                   
                    <Link href="/context"        onClick={(e) => {setTopPosition(e.target.offsetTop); }} className={`${(pathname === "/context")       && ("text-white")} ${showNav ? "left-0" : "left-[600px]"} w-full py-3 relative z-10 h-fit rounded-lg font-medium text-center transition-all ease-in-out duration-[600ms]`}>Context</Link>
